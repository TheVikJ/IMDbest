...
async def on_message(message):
    ...
    if message.content.startswith('!trivia'):
        ...
        for i in range(len(json.loads(r).get('results')[0].get('incorrect_answers'))):
            msg.append(json.loads(r).get('results')[
                       0].get('incorrect_answers')[i])
        msg.sort()
       ...
       #  get-image
        ...
        for i in range(0, 5):
            ...
            response = requests.request(
                "GET", imdburl, headers=headers, params=qstring).text
            image = json.loads(response).get(movieid2).get(
                'title').get('image').get('url')
            title = json.loads(response).get(
                movieid2).get('title').get('title')
            ratings = json.loads(response).get(
                movieid2).get('ratings').get('rating')
            msg = image + '\n' + title + '\nRatings: ' + str(ratings)
            await message.channel.send(msg)
